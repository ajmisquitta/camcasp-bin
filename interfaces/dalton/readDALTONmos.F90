...
!
integer :: NOCCT, NORBT, NBAST ! number of occupied MOs, MOs and basis funcs
real(dp) :: EMCSCF !SCF energy
real(dp) :: POTNUC !Nuclear potential energy
integer :: ndim, norb, nbas, nocc, nvir
...
!The above subroutine doesn't seem to read norb (you get 0). So we read it from
!the SIRIFC file.
...
contains
  ...
  integer, intent(out) :: nbas, norb
  logical, intent(in) :: debug
  integer, intent(out) :: info
  !------------
  character(8), parameter :: label = 'BASINFO '
  integer :: NSYM, NRHF, IOPRHF
  !------------
  info = 0
  ...
  implicit none      
  integer, intent(in) :: iunit, norb, nbas
  ...
  integer, intent(out) :: NOCCT, NORBT, NBAST
  integer, intent(out) :: info
  !------------
  ...
  integer :: NISHT,NASHT,NCONF,NWOPT,NWOPH
  integer :: NCDETS, NCMOT,NNASHX,NNASHY,NNORBT,N2ORBT
  integer :: NSYM,MULD2H, NRHF,NFRO,NISH,NASH,NORB,NBAS
  integer :: NELMN1, NELMX1, NELMN3, NELMX3, MCTYPE,NAS1, NAS2, NAS3
  integer :: MMORBT
  !------------
  info = 0
  !
